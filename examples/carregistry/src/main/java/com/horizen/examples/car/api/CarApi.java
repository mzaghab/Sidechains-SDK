...
import com.horizen.examples.car.box.data.BuyCarOutputDataSource;
...
import com.horizen.examples.car.transaction.*;
...
public class CarApi extends ApplicationApiGroup {
    ...
    private final TransactionCreatorHelper transactionCreatorHelper;
    ...
    */
    ...
    /*
      ...
      Returns the hex representation of the transaction.
    */
    private ApiResponse createCar2(SidechainNodeView view, CreateCarBoxRequest ent) {
        try {
            // Parse the proposition of Car owner.
            PublicKey25519Proposition carOwnershipProposition = PublicKey25519PropositionSerializer.getSerializer()
                .parseBytes(BytesUtils.fromHexString(ent.proposition));
            ...
            TransactionData transactionData
                = new TransactionData(Collections.singletonList(new BoxDataSource<PublicKey25519Proposition>((NoncedBoxData) carBoxData)), ent.fee);

            transactionCreatorHelper.addRegularBoxesForValueWithChange(transactionData, ent.fee, view);


            SidechainTransaction transaction =
                transactionCreatorHelper.createTransaction(transactionData, view, CarDeclarationTransaction2::new, null);
            return new TxResponse(ByteUtils.toHexString(sidechainTransactionsCompanion.toBytes(transaction)));
        }
        catch (Exception e) {
            return new CarResponseError("0102", "Error during Car declaration.", Some.apply(e));
        }
  ...
  Returns the hex representation of the transaction.
*/
    private ApiResponse createCarSellOrder2(SidechainNodeView view, CreateCarSellOrderRequest ent) {
        try {
            // Tre to find CarBox to be opened in the closed boxes list
            CarBox carBox = null;
            ...
            SellOrderProposition sellOrderProposition = new SellOrderProposition(carBox.proposition().pubKeyBytes(), carBuyerProposition.pubKeyBytes());
            CarSellOrderBoxData carSellOrderBoxData = new CarSellOrderBoxData(sellOrderProposition, ent.sellPrice, carBox.getVin(), carBox.getYear(), carBox.getModel(), carBox.getColor());

            TransactionData transactionData =
                new TransactionData(Collections.singletonList(new BoxDataSource(carSellOrderBoxData)), ent.fee);
            transactionCreatorHelper.addRegularBoxesForValueWithChange(transactionData, ent.fee, view);
            SidechainTransaction sidechainTransaction = transactionCreatorHelper.createTransaction(transactionData, view, SellCarTransaction2::new, null);

            return new TxResponse(ByteUtils.toHexString(sidechainTransactionsCompanion.toBytes(sidechainTransaction)));
        }
        catch (Exception e) {
            return new CarResponseError("0102", "Error during Car Sell Order sell operation.", Some.apply(e));
        }
  ...
  Returns the hex representation of the transaction.
*/
    private ApiResponse acceptCarSellOrder2(SidechainNodeView view, SpendCarSellOrderRequest ent) {
        try {
            // Try to find CarSellOrder to be opened in the closed boxes list
            CarSellOrderBox carSellOrderBox = (CarSellOrderBox)view.getNodeState().getClosedBox(BytesUtils.fromHexString(ent.carSellOrderId)).get();
            ...
            //
            boolean isSeller = false;
            Function<byte[], Proof> sellOrderSpendingProofCreator = messageToSign -> {
                ProofOfKnowledge signedMessage = buyerSecretOption.get().sign(messageToSign);
                return new SellOrderSpendingProof(signedMessage.bytes(), isSeller);
            };

            Pair proofCreatorWithBox = new Pair(carSellOrderBox, sellOrderSpendingProofCreator);


            BuyCarOutputDataSource buyCarOutputDataSource = new BuyCarOutputDataSource(carSellOrderBox.getBoxData(), isSeller);
            TransactionData transactionData = new TransactionData(Collections.singletonList(buyCarOutputDataSource), ent.fee, Collections.singletonList(proofCreatorWithBox));
            transactionCreatorHelper.addRegularBoxesForValueWithChange(transactionData, ent.fee, view);

            SidechainTransaction sidechainTransaction = transactionCreatorHelper.createTransaction(transactionData, view, BuyCarTransaction2::new, null);
            //
    ...
    */
    private ApiResponse cancelCarSellOrder(SidechainNodeView view, SpendCarSellOrderRequest ent) {
        try {
            ...
            Function<byte[], Proof> sellOrderSpendingProofCreator = messageToSign -> {
                ProofOfKnowledge signedMessage = ownerSecretOption.get().sign(messageToSign);
                return new SellOrderSpendingProof(signedMessage.bytes(), isSeller);
            };
            ...
            BuyCarOutputDataSource buyCarOutputDataSource = new BuyCarOutputDataSource(carSellOrderBox.getBoxData(), isSeller);
            TransactionData transactionData =
                new TransactionData(Collections.singletonList(buyCarOutputDataSource), ent.fee, Collections.singletonList(proofCreatorWithBox));
            transactionCreatorHelper.addRegularBoxesForValueWithChange(transactionData, ent.fee, view);
  ...
  Returns the hex representation of the transaction.
*/
    private ApiResponse cancelCarSellOrder2(SidechainNodeView view, SpendCarSellOrderRequest ent) {
        try {
            // Try to find CarSellOrder to be opened in the closed boxes list
            Optional<Box> carSellOrderBoxOption = view.getNodeState().getClosedBox(BytesUtils.fromHexString(ent.carSellOrderId));
